...
?>
     ...
     * Delete FCM token
     */
    private function normalize_fcm_token_value($token)
    {
        if (is_string($token)) {
            return $token;
        }
    ...
    public function delete_fcm_token()
    {
        ...
        // Remove token if exists. Older records can store token data as arrays.
        $removed = false;
        foreach ($existing_tokens as $key => $existing_token) {
            if ($this->normalize_fcm_token_value($existing_token) !== $token) {
                continue;
            }
    ...
    public function add_firebase_profile_fields($user)
    {
        ...
        $tokens = array_values(array_filter(array_map(array($this, 'normalize_fcm_token_value'), $tokens)));
