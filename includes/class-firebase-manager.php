...
class FirebaseManager
{
    ...
    private function normalizeTokenValue($token)
    {
        if (is_string($token)) {
            return trim($token);
        }
    ...
    private function normalizeTokenList($tokens)
    {
        if (!is_array($tokens)) {
            return array();
        }

        return array_values(array_unique(array_filter(array_map(array($this, 'normalizeTokenValue'), $tokens))));
    }
    ...
    public function sendNotificationToUser($user_id, $title, $body, $data = array(), $notification_type = 'general')
    {
        ...
        foreach ($tokens as $token) {
            try {
                ...
                $failed_tokens[] = $token;
                error_log('Firebase Push Notifications: Unexpected error sending to token ' . $token . ' - ' . $e->getMessage());
    ...
    private function removeFailedTokens($user_id, $failed_tokens)
    {
        $tokens = get_user_meta($user_id, '_fcm_device_tokens', true);
        if (is_array($tokens)) {
            $tokens = array_values(array_filter($tokens, function ($token) use ($failed_tokens) {
                return !in_array($this->normalizeTokenValue($token), $failed_tokens, true);
            }));
    ...
    public function sendNotificationToAll($title, $body, $data = array(), $notification_type = 'general')
    {
        ...
        if (is_array($guest_tokens)) {
            ...
            foreach ($guest_tokens as $index => $guest_token_data) {
                $token = $this->normalizeTokenValue($guest_token_data);
                if ($token === '') {
                ...
                try {
                    ...
                    $failed_guest_tokens[] = $index;
                    $failed_count++;
                    error_log('Firebase Push Notifications: Unexpected error sending to guest token ' . $token . ' - ' . $e->getMessage());
                }
            }
