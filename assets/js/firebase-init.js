...
(function () {
  ...
  function initializeFirebase() {
    try {
      ...
      // iPhone/iPad web push is available only for Home Screen web apps.
      if (isIOSDevice() && !isStandaloneWebApp()) {
        showIOSSafariMessage();
      } else if (Notification.permission === "default" && !hasBeenAskedForPermission()) {
        // Show permission request for all eligible users if not already asked
  ...
  /**
   * Check whether the visitor has transitioned into a logged-in state.
   */
  function hasJustEnteredLoggedInState() {
    try {
      const authStateKey = 'fcm_auth_state';
      const previousState = localStorage.getItem(authStateKey);
      const currentState = isUserLoggedIn() ? 'logged-in' : 'guest';

      localStorage.setItem(authStateKey, currentState);

      return currentState === 'logged-in' && previousState !== 'logged-in';
    } catch (error) {
      log("Error checking auth state transition: " + error.message);
      return false;
    }
  }
  ...
  function init() {
    ...
    if (
      ...
      try {
        ...
        log("Detected login transition without token, resetting permission prompt flag");
      } catch (error) {
        log("Error resetting permission prompt flag after login: " + error.message);
      }
    }
